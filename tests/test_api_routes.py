...
async def test_status_and_result_are_read_from_state_store(monkeypatch):
    ...
    async def fake_get_status(session_id):
        return "agentic_done"
    ...
    monkeypatch.setattr(routes, "get_status", fake_get_status)
    ...
    assert status_response.json() == {"session_id": "s1", "status": "agentic_done"}
