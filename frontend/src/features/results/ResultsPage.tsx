...
export function ResultsContent({ data, runId, explainEnabled = false }: { data: RunResult; runId: string; explainEnabled?: boolean }) {
  ...
  return (
    <>
      <header className="results-header"><div><p className="eyebrow">统一推荐结果</p><h1>{data.result.summary}</h1><p>共 {roles.length} 个有 JD 证据的岗位；列表不代表录用承诺。</p></div>{explainEnabled ? <Link className="button secondary" to={`/runs/${runId}/evaluation`}>查看评估解释<ArrowUpRight size={17} /></Link> : null}</header>
      ...
      <div className="results-layout">
        ...
        <article className="role-detail">
          ...
          <section><h3>JD 证据</h3><ul className="evidence-inline">{selected.evidence?.map((item) => <li key={item.evidence_span_id}><code>{item.evidence_span_id}</code>{item.content}</li>)}</ul><EvidenceDrawer title={selected.title ?? selected.job_id} evidence={selected.evidence ?? []} resumeEvidence={selected.resume_evidence ?? []} agentMatchReasons={selected.why_this_match ?? []} skillGaps={data.result.skill_gaps ?? []} /></section>
          ...
          <ReactionForm key={selected.job_id} runId={runId} jobId={selected.job_id} />
        </article>
      </div>
...
export function ResultsPage() {
  ...
  useEffect(() => { if (result.data) setProgress({ activeStage: "result", completedStages: ["resume", "intent", "retrieval", "strategy", "verification", "finalization"] }); }, [result.data, setProgress]);
  if (result.isPending) return <section className="loading-state"><h1>正在读取结果</h1></section>;
  if (result.isError) return <section className="notice error" role="alert"><AlertTriangle /><div><h1>结果尚未可读</h1><p>{result.error instanceof ApiError ? recoveryMessage(result.error) : "运行可能仍在处理，返回进度页继续等待。"}</p><Link className="button secondary" to={`/runs/${runId}`}>返回运行</Link></div></section>;
  return <ResultsContent data={result.data} runId={runId} explainEnabled={capabilities.data?.explain_enabled === true} />;
}
