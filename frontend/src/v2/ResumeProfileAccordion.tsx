    ...
    <div className="v2-resume-profile">
      <div className="v2-resume-profile-actions">
        ...
        <span className="v2-resume-reupload-hint">
          {t("需要换文件？用下方输入框左侧的 📎 重新上传")}
        </span>
      </div>

      {open ? (
        <section className="v2-resume-profile-panel" id={panelId} role="region" aria-label={t("完整简历档案")}>
          <section>
            <h3>{t("教育经历")}</h3>
            {education.length ? (
              <div className="v2-resume-records">
                {education.map((item, index) => (
                  <article key={`${item.institution}-${item.dates}-${index}`}>
                    ...
                    <p>{[item.degree, item.field].filter(Boolean).join(" · ") || t("未标注学位与专业")}</p>
                    <small>{item.dates || t("未标注时间")}</small>
                    {item.details?.length ? <ul>{item.details.map((detail) => <li key={detail}>{detail}</li>)}</ul> : null}
                    {item.evidence_span_ids?.length ? <small>{t("证据片段：{ids}", { ids: item.evidence_span_ids.join("、") })}</small> : null}
                  </article>
                ))}
              </div>
            ) : <p className="v2-resume-empty">{t("未识别到教育经历。")}</p>}
          </section>
              ...
              <div className="v2-resume-records">
                {experience.map((item, index) => (
                  <article key={`${item.organization}-${item.title}-${item.dates}-${index}`}>
                    ...
                    <p>{[item.title, item.location].filter(Boolean).join(" · ") || t("未标注职位与地点")}</p>
                    <small>{item.dates || t("未标注时间")}</small>
                    ...
                    {item.technologies?.length ? <p>{t("技术：{technologies}", { technologies: item.technologies.join("、") })}</p> : null}
                    {item.evidence_span_ids?.length ? <small>{t("证据片段：{ids}", { ids: item.evidence_span_ids.join("、") })}</small> : null}
                  </article>
                ))}
              </div>
            ) : <p className="v2-resume-empty">{t("未识别到工作经历。")}</p>}
          </section>
          ...
          <section>
            ...
            ) : <p className="v2-resume-empty">{t("未识别到项目经历。")}</p>}
          </section>
          ...
          <section>
            ...
            ) : <p className="v2-resume-empty">{t("未识别到技能。")}</p>}
          </section>
          ...
          <details className="v2-collapse">
            ...
            ) : <p className="v2-resume-empty">{t("未发现需要提示的档案质量问题。")}</p>}
          </details>
          ...
          <details className="v2-collapse">
            ...
            ) : <p className="v2-resume-empty">{t("没有可展示的原文证据。")}</p>}
          </details>
        </section>
      ) : null}
    </div>
