...
| 12 | 生产短信发送商尚未接入；phone OTP 只有 development/test console hook，production 必须设 `SMS_OTP_PROVIDER=disabled` 并使用 SMTP 邮箱登录 | app/api/auth/providers.py, app/config.py | 当前没有 Twilio/阿里云凭据契约；production 至少保留一个非 disabled 通道，且不能启用 console 泄露验证码 | 选定供应商并补凭据、签名与回执契约后 |
