...
执行分工（**用户 2026-08-06 修正案二**）：**所有批次（前端与后端）统一
为：Codex 执行 → Codex 自查（对照规格逐条核对 + 完整重跑本批门禁并出
自查报告）→ Claude Code 独立审查 → 有问题 **Claude 直接修复** → 修复
推回 **Codex 审查 Claude 的改动** → 如此互审轮回直到双方均无异议
（收敛）**。每轮修复后门禁重跑；收敛后才提交进下批。执行与审查的
独立性由"谁改动、对方审"这一交替规则保障。
